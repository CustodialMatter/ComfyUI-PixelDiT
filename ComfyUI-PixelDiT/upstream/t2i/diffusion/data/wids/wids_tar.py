    ...
    def _create_tar_index(self):
        if self.index_file is not None and os.path.exists(self.index_file):
            ...
            try:
                ...
                if self.verbose:
                    print("Stale pickle-format index detected, regenerating:", self.index_file)
        ...
        if self.index_file is not None:
            ...
            with open(self.index_file + ".temp", "w") as stream:
                json.dump({"fnames": self.fnames, "index": self.index.tolist()}, stream)
