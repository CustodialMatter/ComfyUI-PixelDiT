...
def default_decoder(sample: Dict[str, Any], format: Optional[Union[bool, str]] = True):
    ...
    for key, stream in sample.items():
        ...
        if key.startswith("__"):
            ...
            raise ValueError(
                f"Pickle format is not supported due to security concerns. "
                f"Please convert '{key}' to a safe format (json, msgpack, or npy)."
            )
        elif extension == "mp4":
...
def split_and_recombine(lst, n):
    ...
    part_to_items = {part: [] for part in unique_parts}
    for item in lst:
        part_to_items[extract_prefix(item)].append(item)

    # 2) shuffle within each part using global RNG (seed set elsewhere)
    for items in part_to_items.values():
        random.shuffle(items)

    # 3) split each part into n buckets by stride to keep buckets balanced
    part_to_buckets = {part: [items[i::n] for i in range(n)] for part, items in part_to_items.items()}

    # 4) interleave one-by-one across parts within each bucket index
    parts = unique_parts[:]
    recombined_list = []
    for i in range(n):
        indices = {part: 0 for part in parts}
        lengths = {part: len(part_to_buckets[part][i]) for part in parts}
        remaining = sum(lengths.values())
        while remaining > 0:
            active_parts = [part for part in parts if indices[part] < lengths[part]]
            random.shuffle(active_parts)
            for part in active_parts:
                bucket = part_to_buckets[part][i]
                recombined_list.append(bucket[indices[part]])
                indices[part] += 1
                remaining -= 1
